...
_ROUGE_PATH = '/home/sujal/sem6/btp/final/BTP-1/extractive_summarization/tools/ROUGE-1.5.5'
...
def get_candidates(tokenizer, cls, sep_id, idx):
    ...
    # if len(sent_id) < 2:
    #     indices = [sent_id]
    sent_id = sent_ids[idx]['sent_id'][:6]
    if len(sent_id) < 6:
        indices = [sent_id]  # if fewer than 6, use all
    else:
